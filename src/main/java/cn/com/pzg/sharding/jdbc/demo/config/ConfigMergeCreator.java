...
public class ConfigMergeCreator<C, T> {
    ...
    public T create(C global, C item) throws Exception {
    ...
    private void mergeBasic(C global, C item, T result, PropertyDescriptor pd) throws InvocationTargetException, IllegalAccessException {
        String name = pd.getName();
        Method readMethod = pd.getReadMethod();
